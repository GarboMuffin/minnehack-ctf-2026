...
`);
const createImage = database.prepare(`INSERT INTO images (id, title, content) VALUES (?, ?, ?)`);
const getImage = database.prepare(`SELECT title, content FROM images WHERE id = ?`);
...
app.post('/api/upload', bodyParser.json({
    ...
    if (typeof req.body.title !== 'string') { res.status(400).send('title in body is not a string'); return; }
    if (req.body.title.length > 200) { res.status(400).send('title is too long'); return; }
    if (typeof req.body.content !== 'string') { res.status(400).send('content in body is not a string'); return; }
    if (req.body.content.length > 5000000) { res.status(400).send('content is too long'); return; }

    const id = crypto.randomUUID();

    try {
        createImage.run(id, req.body.title, req.body.content);
    } catch (e) {
        console.error(e);
...
app.get('/api/get', (req, res) => {
    ...
    res.json({ title: response.title, content: response.content });
});
